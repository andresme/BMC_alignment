...
void runNeedlemanWunsch(enum GAP_TYPE v_type, enum GAP_TYPE w_type, char *v_string, char *w_string,
                        enum ALIGNMENT_MODE mode, int threads, int initial_k, int adjust_k) {
    float times[threads];
    ...
    for (wave = 1; wave <= td->imax + td->numThreads - 1; wave++) {
        ...
        if (i >= 1 && i <= td->imax) {
            for (j = tStart; j <= tEnd; j++) {
                ...
                temp[0] = H[i - 1][j - 1] != INT_MIN ? H[i - 1][j - 1] + similarity_score(seq_w[i - 1], seq_v[j - 1])
                                                     : INT_MIN;
    ...
    for (wave = 1; wave <= td->imax + td->numThreads - 1; wave++) {
        ...
        if (i >= 1 && i <= td->imax) {
            for (j = tStart; j <= tEnd; j++) {
                ...
                tempH[0] = H[i - 1][j - 1] != INT_MIN ? H[i - 1][j - 1] + similarity_score(seq_w[i - 1], seq_v[j - 1])
                                                      : INT_MIN;
                tempH[1] = B[i - 1][j - 1] != INT_MIN ? B[i - 1][j - 1] + similarity_score(seq_w[i - 1], seq_v[j - 1])
                                                      : INT_MIN;
                tempH[2] = C[i - 1][j - 1] != INT_MIN ? C[i - 1][j - 1] + similarity_score(seq_w[i - 1], seq_v[j - 1])
                                                      : INT_MIN;
                tempH[3] = 0;
                ...
                tempB[0] = H[i - 1][j] != INT_MIN ? H[i - 1][j] + score_table.continue_block_cost +
                                                    score_table.new_block_cost : INT_MIN;
                tempB[1] = B[i - 1][j] != INT_MIN ? B[i - 1][j] + score_table.continue_block_cost : INT_MIN;
                tempB[2] = C[i - 1][j] != INT_MIN ? C[i - 1][j] + score_table.continue_block_cost +
                                                    score_table.new_block_cost : INT_MIN;
                tempB[3] = 0;
                ...
                tempC[0] = H[i][j - 1] != INT_MIN ? H[i][j - 1] + score_table.continue_block_cost +
                                                    score_table.new_block_cost : INT_MIN;
                tempC[1] = B[i][j - 1] != INT_MIN ? B[i][j - 1] + score_table.continue_block_cost +
                                                    score_table.new_block_cost : INT_MIN;
